...
const char * const AFCO_CONF_FILENAME = "afco.conf";
const char * const AFCO_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/afco/afco/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //afco-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", AFCO_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\AFCO
    // macOS: ~/Library/Application Support/AFCO
    // Unix-like: ~/.afco
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AFCO";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/AFCO";
#else
    // Unix-like
    return pathRet / ".afco";
#endif
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", AFCO_CONF_FILENAME);
