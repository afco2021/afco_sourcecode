...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<AFCOAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_afco-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_afco-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_afco-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = AFCOUnits::formatWithUnit(unit, balance, false, AFCOUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    AFCOAmountField* amountInput = receiveCoinsDialog.findChild<AFCOAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_afco-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
