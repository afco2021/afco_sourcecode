...
#include <qt/afcoaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a AFCO address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new AFCOAddressEntryValidator(parent));
    widget->setCheckValidator(new AFCOAddressCheckValidator(parent));
}

bool parseAFCOURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no afco: URI
    if(!uri.isValid() || uri.scheme() != QString("afco"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!AFCOUnits::parse(AFCOUnits::AFCO, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseAFCOURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseAFCOURI(uriInstance, out);
}

QString formatAFCOURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("afco:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(AFCOUnits::format(AFCOUnits::AFCO, info.amount, false, AFCOUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openAFCOConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", AFCO_CONF_FILENAME));
    ...
    /* Open afco.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "AFCO.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "AFCO (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("AFCO (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "afco.desktop";
    return GetAutostartDir() / strprintf("afco-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a afco.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=AFCO\n";
        else
            optionFile << strprintf("Name=AFCO (%s)\n", chain);
