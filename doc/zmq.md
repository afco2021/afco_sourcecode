...
The AFCO Core daemon can be configured to act as a trusted "border
router", implementing the afco wire protocol and relay, making
...
The ZeroMQ feature in AFCO Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building afcod:
...
For instance:
    ...
    $ afcod -zmqpubhashtx=tcp://127.0.0.1:233583 \
               -zmqpubhashtx=tcp://192.168.1.2:233583 \
               -zmqpubrawtx=ipc:///tmp/afcod.tx.raw \
...
These options can also be provided in afco.conf.
...
From the perspective of afcod, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into afcod directly. Furthermore, no information is
...
using. AFCOd appends an up-counting sequence number to each
